...
(function () {
  ...
  window.finalizeQuestions = function () {
    ...
    run(async () => {
      ...
      // 유형마다 하나씩, 그리고 '질문 더하기' 로 적은 것을 차례로 공유합니다.
      for (const o of list) {
        await A.inquiry.share(state.sSessionId, SELF.id, o.text, o.k, rid, false);
      }
      for (const t of extras) {
        await A.inquiry.share(state.sSessionId, SELF.id, t, "", rid, true);
      }
      ...
      saveInqRound();
      await refreshShared();
      toast(`${label} 질문 ${list.length + extras.length}개를 공유했어요.`);
      setStudentTab("p4");
    });
  };
  window.reopenInqRound = function () {
    ...
    toast("다시 열었어요. 고친 뒤 한 번 더 공유하면 새 질문으로 올라가요.");
  };
