...
(function () {
  ...
  const sb = window.supabase.createClient(CFG.SUPABASE_URL, CFG.SUPABASE_ANON_KEY, {
    auth: { persistSession: true, autoRefreshToken: true, storageKey: "qlab-auth" },
  });
  ...
  const sessions = {
    ...
    async create(className, topic) {
      const { data, error } = await sb.rpc("create_session", {
        p_class_name: className, p_topic: topic,
      });
      fail(error);
      return data;
    },
  ...
  const inquiry = {
    ...
    async saveDraft(sessionId, studentId, roundId, patch) {
      const { error } = await sb.from("inquiry_drafts")
  ...
  const essay = {
    ...
    async sessionRows(sessionId) {
      const { data, error } = await sb.from("essays")
        .select("student_id, round_id, submitted, body, steps")
        .eq("session_id", sessionId);
      fail(error);
      return data || [];
    },
  ...
  window.QLab = {
    sb, fmtDate, fmtDateTime, callFunction,
    auth, adminCodes, sessions, students, words, materials, storage,
