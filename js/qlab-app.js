...
(function () {
  ...
  async function hydrateStudentData() {
    ...
    subscribe(sid);
    startGateWatch();   // 실시간이 끊겨도 닫힘을 놓치지 않게 하는 안전망
  }
  ...
  async function fillRoundEssays(sid, d) {
    ...
    rows.forEach((r) => {
      (byStudent[r.student_id] ||= {})[r.round_id] = {
        ...
        // 전체 내용. [학생 모니터링] 에서 글 전체를 볼 때 씁니다.
        //   예전에는 앞 60자만 남겨 두어서, 학생이 쓰다 만 글을 끝까지 볼 수 없었습니다.
        essayBody: r.body || "",
        essayPreview: (r.body || "").slice(0, 60),
        essayChars: String(r.body || "").trim().length,
  ...
  function subscribe(sessionId) {
    ...
    channel = A.sb.channel(`session-${sessionId}`)
      ...
      .on("postgres_changes",
        ...
        (payload) => {
          ...
          const next = {
            ...
            inquiryOpen: row.inquiry_open !== false,
          };
          const wasOpen = idx >= 0 ? list[idx].essayOpen === true : false;
          const wasInq  = idx >= 0 ? list[idx].inquiryOpen !== false : true;
          if (idx >= 0) list[idx] = next; else list.push(next);

          if (ROLE === "student" && next.inquiryOpen !== wasInq) {
            if (next.inquiryOpen) {
              reopenNotice("inq", next.id);
              toast(`선생님이 ${next.label} 질문 만들기를 열었어요.`);
            } else {
              closeInquiryNow(next);
            }
          }

          if (ROLE === "student" && next.essayOpen !== wasOpen) {
            if (next.essayOpen) {
              // 여는 것만으로는 서버의 글이 지워지지 않으므로,
              // 화면에 써 둔 글도 그대로 둡니다. 이어서 쓸 수 있습니다.
              // (선생님이 '글 초기화' 를 따로 누르면 아래 essays 변경으로 갱신됩니다)
              reopenNotice("essay", next.id);
      ...
      // 선생님이 '글 초기화' 를 누르면 그 차시의 essays 행이 지워집니다.
      //   학생 화면에 남아 있는 글도 함께 비워야, 이어서 쓰다가 되살아나지 않습니다.
      .on("postgres_changes",
        { event: "DELETE", schema: "public", table: "essays" },
        (payload) => {
          if (ROLE !== "student") return;
          const gone = payload.old || {};
          if (SELF && gone.student_id && gone.student_id !== SELF.id) return;
          const rid = gone.round_id;
          if (!rid) return;
          delete state.essayRounds[rid];
          if (state.essayRoundId === rid) {
            applyEssay(essayBlank());
            toast("선생님이 이 차시의 글을 초기화했어요.");
          }
          render();
        })
      .on("postgres_changes",
  ...
  /* =======================================================
     차시가 닫혔을 때 학생 화면을 '읽기 전용' 으로 바꾸는 곳

     닫힘 소식은 두 길로 들어옵니다.
       ① 실시간(subscribe) — 보통은 이쪽이 즉시 도착합니다.
       ② 감시(watchRoundGates) — 실시간이 끊기거나 늦을 때를 위한 안전망.
     두 길 모두 아래 두 함수만 부르게 해서, 어느 쪽으로 들어와도
     학생이 보는 결과가 똑같도록 맞췄습니다.

     같은 차시에 대해 두 번 알리지 않도록 '이미 알렸다' 를 기억해 둡니다.
     ======================================================= */
  const closedNotified = { inq: {}, essay: {} };

  // 선생님이 다시 열면, 다음에 닫을 때 또 알릴 수 있게 되돌립니다.
  function reopenNotice(kind, roundId) {
    delete closedNotified[kind][roundId];
  }

  // 화면이 들고 있는 차시 목록에서 그 차시의 여닫기 상태만 갈아 끼웁니다.
  function markRoundGate(roundId, patch) {
    const d = bucket(state.sSessionId);
    const list = roundsOf(d);
    const r = list.find((x) => x.id === roundId);
    if (r) Object.assign(r, patch);
    return r;
  }

  /* [질문 만들기] 가 닫혔을 때.
     · 곧바로 읽기 전용으로 바꾸고 알림창을 띄웁니다.
     · 서버에서 정리된 결과(자동 공유 등)를 받아 오는 일은 그 뒤에 합니다.
       인터넷이 잠깐 끊겨 그 요청이 실패하더라도, 잠금과 알림창은
       이미 끝나 있어야 하기 때문입니다. (예전에는 여기서 막히면
       알림창이 아예 뜨지 않았습니다) */
  function closeInquiryNow(round) {
    if (ROLE !== "student" || !round) return;
    if (closedNotified.inq[round.id]) return;
    closedNotified.inq[round.id] = true;

    markRoundGate(round.id, { inquiryOpen: false });
    const mine = state.inqRoundId === round.id;

    // ① 먼저 화면을 잠급니다.
    render();
    if (mine) showRoundClosedModal("inq", round.label, !!state.inqSubmitted);
    toast(`선생님이 ${round.label} 질문 만들기를 닫았어요.`);

    // ② 그 다음에 서버가 정리한 내용을 받아 옵니다. (실패해도 잠금은 그대로)
    run(async () => {
      const drafts = await A.inquiry.myDrafts(SELF.id);
      (drafts || []).forEach((row) => {
        if (row.round_id) {
          state.inqRounds[row.round_id] = draftToSnapshot(row, state.sSessionId);
        }
      });
      if (state.inqRoundId === round.id) {
        applyInq(state.inqRounds[round.id] || inqBlank());
      }
      await refreshShared();
      render();
    });
  ...
  /* [글쓰기] 가 닫혔을 때.
     집중 모드와 이탈 경고창도 함께 풀어 줍니다.
     전체화면에 갇힌 채로 두면 제출도 막혀 있어서 나올 방법이 없습니다. */
  function closeEssayNow(round) {
    if (ROLE !== "student" || !round) return;
    if (closedNotified.essay[round.id]) return;
    closedNotified.essay[round.id] = true;

    markRoundGate(round.id, { essayOpen: false });
    const mine = state.essayRoundId === round.id;

    if (mine) {
      leaveFocusAfterSubmit();   // 집중 모드 · 전체화면 풀기
      releaseLeaveModal();       // 이탈 경고창이 떠 있으면 함께 닫기
    }
    // ① 먼저 화면을 잠급니다.
    render();
    if (mine) showRoundClosedModal("essay", round.label, !!state.essaySubmitted);
    toast(`선생님이 ${round.label} 글쓰기를 닫았어요.`);

    // ② 그 다음에 서버가 정리한 글(자동 제출 · 되돌리기)을 받아 옵니다.
    if (!mine) return;
    run(async () => {
      const rows = await A.essay.mineAll(SELF.id);
      (rows || []).forEach((row) => {
        if (row.round_id) state.essayRounds[row.round_id] = essayRowToSnapshot(row);
      });
      applyEssay(state.essayRounds[round.id] || essayBlank());
      render();
    });
  }

  /* ---------- 차시 여닫기 감시 (가벼운 안전망) ----------
     실시간 연결이 끊기거나 소식이 늦으면, 선생님이 닫아도 학생 화면은
     그대로 열려 있었습니다. 그래서 짧은 확인을 하나 더 둡니다.

     [무겁지 않게 하려고 지킨 것들]
       · 학생 화면에서만 돕니다. (선생님 화면은 돌지 않습니다)
       · [질문 만들기] · [글쓰기] 탭을 보고 있을 때만 물어봅니다.
       · 그 탭의 차시가 이미 닫혀 있으면 물어보지 않습니다.
       · 다른 앱을 보고 있으면(document.hidden) 건너뜁니다.
       · 받아 오는 것은 id · 여닫기 두 칸뿐입니다. (주제 · 유형은 안 받음)
       · 바뀐 것이 없으면 화면을 다시 그리지 않습니다.
         (글을 쓰는 중에 다시 그리면 커서가 튀기 때문입니다)
     그래서 실제로는 '해당 탭에 머무는 동안 7초마다 수백 바이트' 입니다. */
  const GATE_WATCH_MS = 7000;
  let gateWatch = null;
  let gateBusy = false;

  function gateWatchNeeded() {
    if (ROLE !== "student" || !state.sSessionId) return null;
    if (state.route !== "student-app") return null;
    if (typeof document !== "undefined" && document.hidden) return null;
    // 지금 보고 있는 탭에서 쓰이는 차시만 확인합니다.
    if (state.studentTab === "p3" && state.p3Tab === "make") {
      return inquiryOpen() ? "inq" : null;      // 이미 닫혔으면 더 물어볼 것이 없습니다
    }
    if (state.studentTab === "p6") {
      return essayOpen() ? "essay" : null;
    }
    return null;
  }

  async function checkRoundGates(kind) {
    const sid = state.sSessionId;
    const gates = await A.rounds.gates(sid);
    const list = roundsOf(bucket(sid));
    // 닫힘은 close...Now 가 스스로 화면을 다시 그립니다.
    // 여기서 세는 것은 '열림' 처럼 아직 다시 그리지 않은 변화뿐입니다.
    let needRender = false;

    gates.forEach((g) => {
      const r = list.find((x) => x.id === g.id);
      if (!r) return;
      const wasInq = r.inquiryOpen !== false;
      const wasEssay = r.essayOpen === true;

      if (wasInq && !g.inquiryOpen) {
        closeInquiryNow({ id: r.id, label: r.label });
      } else if (!wasInq && g.inquiryOpen) {
        r.inquiryOpen = true; reopenNotice("inq", r.id); needRender = true;
      }

      if (wasEssay && !g.essayOpen) {
        closeEssayNow({ id: r.id, label: r.label });
      } else if (!wasEssay && g.essayOpen) {
        r.essayOpen = true; reopenNotice("essay", r.id); needRender = true;
      }
    });

    // 바뀐 것이 없으면 화면은 손대지 않습니다.
    //   글을 쓰는 중에 괜히 다시 그리면 커서가 튀기 때문입니다.
  ...
  function startGateWatch() {
    ...
    gateWatch = setInterval(() => {
      const kind = gateWatchNeeded();
      if (!kind || gateBusy) return;
      gateBusy = true;
      checkRoundGates(kind)
        .catch((e) => console.warn("차시 확인 실패", e))   // 인터넷이 끊겼을 뿐이면 다음에 다시 봅니다
        .finally(() => { gateBusy = false; });
    }, GATE_WATCH_MS);
  }

  /* 다른 앱을 보다가 돌아왔을 때는 기다리지 않고 곧바로 한 번 확인합니다.
     쉬는 시간에 화면을 덮어 두었다가 여는 상황이 잦기 때문입니다. */
  if (typeof document !== "undefined") {
    document.addEventListener("visibilitychange", () => {
      ...
      gateBusy = true;
      checkRoundGates(kind)
        .catch((e) => console.warn("차시 확인 실패", e))
        .finally(() => { gateBusy = false; });
    });
  }
  ...
  /* [브레인스토밍] 지우기.
     index.html 의 미리보기용 함수를 덮어씁니다.
     지울 대상은 state.wordDel 에 담겨 있습니다. (askDeleteWord... 가 넣어 둡니다)

     지운 뒤에는 목록을 서버에서 다시 받아 옵니다.
     화면에서만 지우면, 다른 기기에서 보고 있는 선생님 화면과 어긋납니다. */
  window.confirmWordDelete = function () {
    const job = state.wordDel;
    ...
    run(async () => {
      if (job.mode === "all") {
        const n = await A.words.clearAll(sid);
        toast(`브레인스토밍 ${n}건을 지웠어요.`);
      } else if (job.mode === "word") {
        await A.words.removeWord(sid, job.text);
        toast("질문을 지웠어요.");
      } else {
        // 한 학생이 같은 질문을 여러 번 썼을 수도 있어 줄마다 지웁니다.
        for (const id of job.ids) await A.words.removeOne(id);
        toast("질문을 지웠어요.");
      }
      await refreshWords();   // 서버 기준으로 목록을 다시 맞춥니다
    });
  };
  /* 이탈 경고창이 떠 있는 동안에는 5초마다 차시 설정을 확인합니다.
     실시간 소식이 늦거나 끊겨도, 선생님이 닫으면 학생이 풀려나야 하기 때문입니다.
     이 창은 스스로 닫을 수 없어서, 못 빠져나오면 수업이 멈춥니다. */
  let leaveWatch = null;
  // index.html 쪽에서 부르므로 window 에 붙여 둡니다.
  //   (이 파일 전체가 하나의 함수 안이라, 그냥 두면 밖에서 보이지 않습니다)
  window.watchLeaveModal = function watchLeaveModal() {
    clearInterval(leaveWatch);
    leaveWatch = setInterval(async () => {
      if (!leaveModalUp) { clearInterval(leaveWatch); leaveWatch = null; return; }
      try {
        await refreshRounds();
        if (!essayOpen()) releaseLeaveModal();
      } catch (e) {
        console.warn("차시 확인 실패", e);   // 인터넷이 끊겼을 뿐이면 다음에 다시 봅니다
      }
    }, 5000);
  };

  // 학생이 스스로 확인해 볼 수 있는 길.
  //   선생님이 닫았으면 창을 풀어 주고, 아직 열려 있으면 그대로 둡니다.
  //   (아무것도 못 하고 갇히는 상황을 막기 위한 마지막 안전장치입니다)
  window.checkEssayStillOpen = function () {
    run(async () => {
      await refreshRounds();
      if (!essayOpen()) { releaseLeaveModal(); return; }
      toast("아직 글쓰기가 열려 있어요. 집중 모드로 돌아가 주세요.");
    });
  };

  // 차시 설정(여닫기 · 주제 · 질문 유형)만 다시 받아 옵니다.
  async function refreshRounds() {
    const sid = ROLE === "student" ? state.sSessionId : state.tSessionId;
    if (!sid) return;
    bucket(sid).rounds = await A.rounds.list(sid);
    render();
  }
  ...
  window.setStudentTab = function (k) {
    ...
    // 실시간 연결이 끊겼거나 늦을 수 있으므로, 이 두 탭에 들어갈 때는
    // 차시가 아직 열려 있는지 서버에 다시 확인합니다.
    if (k === "p3" || k === "p6") run(refreshRounds);
  ...
  window.saveStep = function (n) {
    /* 닫힌 차시에서는 [다음] 으로 단계를 넘기지 않습니다.
       예전에는 여기서 goStep(n+1) 로 그냥 넘겨 주었는데,
       그 바람에 STEP 2-1 에서 2-2(AI 피드백) 로 들어갈 수 있었습니다.
       읽기만 하려는 학생은 위의 단계 표시(●)를 눌러 둘러볼 수 있습니다. */
    if (!inquiryOpen()) { noticeInquiryClosed(); return; }

    /* STEP 2-1 -> 2-2 는 AI 피드백 화면으로 들어가는 문입니다.
       실시간 소식이 아주 잠깐 늦을 수 있으므로, 이 문 앞에서만
       서버에 한 번 더 확인합니다. (id·여닫기 두 칸짜리 가벼운 확인) */
    if (n === 2) {
      // 빈칸 확인은 먼저 합니다. 안 적었는데 서버까지 다녀오게 할 이유가 없습니다.
      if (!filledStep2().length) { toast("질문을 한 개 이상 적어주세요."); return; }
      run(async () => {
        await checkRoundGates("inq");
        if (!inquiryOpen()) { noticeInquiryClosed(); return; }
        goStep2To3();
      });
      return;
  ...
  // STEP 2-1 에서 2-2 로 넘어가며 지금까지 쓴 질문을 저장합니다.
  function goStep2To3() {
  ...
  /* 닫혔다는 사실을 학생에게 확실히 알립니다.
     아직 알림창을 본 적이 없으면 창까지 띄우고, 이미 봤으면 한 줄 안내만 합니다. */
  function noticeInquiryClosed() {
    const label = roundLabel(sData(), state.inqRoundId);
    if (!closedNotified.inq[state.inqRoundId]) {
      closedNotified.inq[state.inqRoundId] = true;
      render();
      showRoundClosedModal("inq", label, !!state.inqSubmitted);
      return;
    }
    toast("선생님이 이 차시의 질문 만들기를 닫았어요. 지금은 읽기만 할 수 있어요.");
  }

  // [읽기 전용] 버튼에서 부릅니다. (index.html 의 같은 이름 함수를 덮어씁니다)
  window.noticeInquiryClosedFromUI = function () { noticeInquiryClosed(); };

  // 닫힌 차시인지 확인하고, 닫혔으면 안내만 하고 멈춥니다.
  function blockedIfClosed() {
    if (inquiryOpen()) return false;
    noticeInquiryClosed();
    return true;
  }

  window.runAIEval = function () {
    if (blockedIfClosed()) return;
    state.stepView = 3;
    ...
    /* AI 를 부르기 직전에 서버에 한 번 더 확인합니다.
       어차피 AI 응답을 기다려야 해서, 이 짧은 확인은 학생이 느끼지 못합니다.
       (서버 쪽에서도 닫힌 차시는 거절하므로, 이건 화면을 먼저 잠그기 위한 것입니다) */
    checkRoundGates("inq")
      .catch(() => {})                // 인터넷이 잠깐 흔들린 것뿐이면 그냥 진행합니다
      .then(() => {
        if (!inquiryOpen()) {
          state.step3.phase = "idle";
          noticeInquiryClosed();
          return null;
        }
        return A.inquiry.evaluate(state.step3.question, list, state.inqRoundId);
      })
      .then((r) => {
        if (!r) return;               // 위에서 닫힘으로 멈춘 경우
  ...
  // '내 생각 정리하기' 칸의 두 이유를 하나의 글로 묶어 보냅니다.
  window.sendDiscussionReasons = function () {
    const msg = buildReasonMessage();
    if (!msg) return;
    sendToAI({ message: msg });
  };
  ...
  // 차시별 [질문 만들기] 열기 / 닫기
  //   닫아 두면 학생은 그 차시에서 쓰기·고치기·지우기를 할 수 없습니다.
  //   화면에서 감출 뿐 아니라 창고에서도 막습니다. (14_updates.sql)
  window.toggleInquiryOpen = function () {
    const r = tRound();
    if (!r) return;
    const on = r.inquiryOpen === false;   // 지금 닫혀 있으면 연다
    run(async () => {
      await A.rounds.update(r.id, { inquiryOpen: on });
      r.inquiryOpen = on;

      // 닫을 때 정리합니다.
      //   · 안 낸 학생  → 만들어 둔 최종 질문을 그대로 공유 처리
      //   · 고치던 학생 → 마지막으로 냈던 내용으로 되돌림
      let fixed = 0;
      if (!on) {
        try { fixed = await A.inquiry.finalizeRound(r.id); }
        catch (e) { console.warn("질문 정리 실패", e); }
      }
      await refreshShared();
      await refreshStudents();
      render();
      toast(on
        ? `${r.label} 질문 만들기를 열었어요.`
        : (fixed > 0
            ? `${r.label} 질문 만들기를 닫고, 학생 ${fixed}명의 질문을 정리했어요.`
            : `${r.label} 질문 만들기를 닫았어요.`));
    });
  };

  // 글쓰기 열기 / 잠그기. 써 둔 글은 건드리지 않습니다.
  window.applyEssayDetect = function (on) {
    ...
    run(async () => {
      ...
      // 닫을 때는 학생들의 글을 정리합니다.
      //   · 안 낸 학생   → 그때까지 쓴 글을 그대로 제출 처리
      //   · 고치던 학생  → 마지막으로 냈던 글로 되돌림
      //   창고에서 한꺼번에 하므로, 기기가 꺼져 있는 학생도 함께 정리됩니다.
      let fixed = 0;
      if (!on) {
        try { fixed = await A.essay.finalizeRound(r.id); }
        catch (e) { console.warn("글 정리 실패", e); }
      }
      await refreshStudents();
      render();
      toast(on
        ? `${r.label} 글쓰기를 열었어요.`
        : (fixed > 0
            ? `${r.label} 글쓰기를 닫고, 쓰던 글 ${fixed}건을 정리했어요.`
            : `${r.label} 글쓰기를 닫았어요.`));
    });
  };

  // 이 차시의 글만 지웁니다. 여닫기와는 따로 움직입니다.
  window.confirmClearEssays = function () {
    const sid = state.tSessionId;
    const r = tRound();
    if (!r) return;
    run(async () => {
      const cleared = await A.essay.clearSession(sid, r.id);
      closeModal();
      await refreshStudents();
      render();
      toast(cleared > 0
        ? `${r.label} 글 ${cleared}건을 지웠어요.`
        : `${r.label}에는 지울 글이 없었어요.`);
    });
  };
  ...
  window.saveFeedback = function (id, area) {
    const key = area || "all";
    const box = $(`#fb-${key}`);
    if (!box) return;
    const v = box.value.trim();
    run(async () => {
      await A.students.setFeedback(id, v, key);
      await refreshStudents();
      renderStudentDetail(id);     // 시트를 닫지 않고 그 자리에서 갱신합니다
      toast("피드백을 저장했어요.");
    });
  };
  ...
  window.collectStudentEvidence = function (s) {
    return {
      ...
      // 아직 안 낸 글도 '얼마나 썼는지' 는 보여 줘야 합니다.
      essayChars: Number(s.essayChars) || 0,
