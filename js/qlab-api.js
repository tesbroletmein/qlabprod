...
(function () {
  ...
  const students = {
    ...
    async listForTeacher(sessionId) {
      ...
      return (data || []).map((s) => ({
        ...
        // essayBody(전체 내용)는 위의 ...s 로 그대로 들어옵니다.
        //   미리보기는 목록에서 짧게 보여 줄 때만 씁니다.
  ...
  const words = {
    ...
    async log(sessionId) { // 교사 전용
      ...
      (data || []).forEach((r) => {
        ...
        grouped.get(r.word).by.push({
          id: r.id,                 // 이 줄만 콕 집어 지울 때 씁니다
          who: r.nickname,
    ...
    /* 아래 셋은 선생님만 쓸 수 있습니다.
       막는 일은 창고 쪽 규칙(words_delete)이 하므로 여기서 또 확인하지 않습니다.
       학생이 몰래 불러도 자기가 쓴 줄 말고는 지워지지 않습니다. */

    // 한 줄만 지웁니다. (어떤 학생이 쓴 질문 하나)
    async removeOne(id) {
      const { error } = await sb.from("words").delete().eq("id", id);
      fail(error);
    },
    // 같은 질문을 지웁니다. 여러 학생이 똑같이 썼으면 그것들까지 함께 지웁니다.
    async removeWord(sessionId, word) {
      const { error } = await sb.from("words").delete()
        .eq("session_id", sessionId).eq("word", word);
      fail(error);
    },
    // 이 세션의 브레인스토밍을 통째로 비웁니다. 지운 개수를 돌려줍니다.
    async clearAll(sessionId) {
      const { data, error } = await sb.rpc("clear_session_words", { p_session: sessionId });
      fail(error);
  ...
  const rounds = {
    ...
    /* 차시의 '여닫기' 상태만 아주 가볍게 확인합니다.
       몇 초마다 부르는 감시용이라, 주제·질문 유형 같은 무거운 칸은 빼고
       꼭 필요한 세 칸만 받아 옵니다. (차시가 5개여도 한 번에 수백 바이트) */
    async gates(sessionId) {
      const { data, error } = await sb.from("session_rounds")
        .select("id, essay_open, inquiry_open")
        .eq("session_id", sessionId);
      fail(error);
      return (data || []).map((r) => ({
        id: r.id,
        essayOpen: r.essay_open === true,
        inquiryOpen: r.inquiry_open !== false,
      }));
    },
  ...
  /* ---------- 탐구 질문 ---------- */
  /* 질문 유형의 우리말 이름.
     index.html 의 STEP2_TYPES 와 같은 이름을 씁니다.
     유형을 더하거나 이름을 바꾸면 두 곳을 함께 고쳐 주세요. */
  const QTYPE_LABEL = {
    reflective: "성찰적 질문",
    debate:     "논쟁적 질문",
    problem:    "문제 해결을 위한 질문",
  };
  ...
  const inquiry = {
    ...
    async feed(sessionId) {
      ...
      fail(error);
      /* 창고는 유형을 'reflective' 같은 이름표(qtype)로만 내려보냅니다.
         화면은 '성찰적 질문' 처럼 우리말 이름(qtypeLabel)을 찾아 쓰므로
         여기에서 짝을 지어 붙여 줍니다.
         이게 없어서 [학생 모니터링]과 [친구들 질문]에 유형이 안 보였습니다. */
      return (data || []).map((q) => ({
        ...q,
        comments: q.comments || [],
        qtypeLabel: QTYPE_LABEL[q.qtype] || "",
      }));
    },
