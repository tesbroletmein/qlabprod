...
(function () {
      ...
      .on("postgres_changes",
        ...
        (payload) => {
          ...
          if (ROLE === "student" && next.inquiryOpen !== wasInq) {
            if (!next.inquiryOpen) {
              // 서버에서 정리된 결과(자동 공유 또는 되돌리기)를 다시 받아 옵니다.
              run(async () => {
                const drafts = await A.inquiry.myDrafts(SELF.id);
                (drafts || []).forEach((row) => {
                  if (row.round_id) state.inqRounds[row.round_id] = draftToSnapshot(row, sessionId);
                });
                if (state.inqRoundId === next.id) {
                  applyInq(state.inqRounds[next.id] || inqBlank());
                }
                await refreshShared();
                render();
                // 지금 그 차시를 보고 있으면 창을 띄워 확실히 알려 줍니다.
                if (state.inqRoundId === next.id) {
                  showRoundClosedModal("inq", next.label, !!state.inqSubmitted);
                }
              });
            }
            toast(next.inquiryOpen
              ? `선생님이 ${next.label} 질문 만들기를 열었어요.`
              : `선생님이 ${next.label} 질문 만들기를 닫았어요.`);
          }
          ...
          if (ROLE === "student" && next.essayOpen !== wasOpen) {
            if (next.essayOpen) {
              ...
              // 닫히면 집중 모드에서 빠져나오게 합니다.
              //   전체화면에 갇힌 채로 두면, 제출도 막혀 있어서
              //   학생이 화면에서 나올 방법이 없습니다.
              if (state.essayRoundId === next.id) {
                // 서버에서 정리된 결과(자동 제출 또는 되돌리기)를 다시 받아 옵니다.
                run(async () => {
                  const rows = await A.essay.mineAll(SELF.id);
                  (rows || []).forEach((row) => {
                    if (row.round_id) state.essayRounds[row.round_id] = essayRowToSnapshot(row);
                  });
                  // 지금 보고 있는 차시라면 화면에도 바로 반영합니다.
                  applyEssay(state.essayRounds[next.id] || essayBlank());
                  render();
                  showRoundClosedModal("essay", next.label, !!state.essaySubmitted);
                });
                leaveFocusAfterSubmit();
                // 이탈 경고창이 떠 있으면 그 창도 닫아 줍니다.
                //   이 창은 '집중 모드로 돌아가기' 말고는 닫을 방법이 없어서,
                //   그냥 두면 학생이 화면에 갇힙니다.
                releaseLeaveModal();
              }
              if (state.studentTab === "p6" && state.essayRoundId === next.id) {
  ...
  window.saveStep = function (n) {
    // 닫힌 차시에서는 저장하지 않고 넘어가기만 합니다.
    //   [다음] 이 이 함수를 부르기 때문에, 그냥 막으면 학생이 앞으로 못 갑니다.
    if (!inquiryOpen()) { goStep(n + 1); return; }
    if (n === 1) {
